package com.example.mythesis.office;
...
public class Gudang_Arsip extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        title.setText("Gudang Arsip");
        tempat = (TextView) findViewById(R.id.tempat);
        tempat.setText("Gudang Arsip");
    ...
    private final SensorEventListener LightSensorListener = new SensorEventListener()
    {
        ...
        public void onSensorChanged(SensorEvent event)
        {
            ...
            if ((+event.values[0]) <= 5)
            {
                AlertDialog.Builder builder = new AlertDialog.Builder(Gudang_Arsip.this);
                builder.setTitle("Perhatian");
                builder.setMessage(" letakkan ponsel di tempat yang cukup cahaya konoyaro !");
                builder.setCancelable(true);
                final AlertDialog dlg = builder.create();
                dlg.show();
                Handler mHandler = new Gudang_Arsip.handler();
...
//            }
            else if ((+event.values[0]) > 5 && (+event.values[0]) <= 30)
            {
                area.setText(" GUDANG ARSIP ");
            ...
            else if ((+event.values[0]) > 30 && (+event.values[0]) <= 60)
            {
                area.setText(" GUDANG ARSIP ");
            ...
            else if ((+event.values[0]) > 90 && (+event.values[0]) <= 120)
            {
                area.setText(" GUDANG ARSIP ");
                status.setText(" HAMPIR MEMENUHI SYARAT ");
                rekomendasi.setText(" pencahayaan masih kurang, ganti dengan lampu antara 7 watt sampai 20 watt");
            }
            else if ((+event.values[0]) > 120 && (+event.values[0]) <= 150)
            {
                area.setText(" GUDANG ARSIP ");
